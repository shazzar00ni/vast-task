...
- **Every dispatch runs in a fresh `git worktree`** (branch `vastask/task-<issue-number>-<short-id>`, cut from a freshly-fetched default branch, in a directory outside the normal working tree — for a local-only task with no GitHub issue, `<issue-number>` falls back to the task's local id). A run must proceed normally even with uncommitted changes sitting in the primary working directory, and never touch it — this is flagged repeatedly as the single most important behavior to verify by test, not assume.
